...
#
# Usage: moves2smkEF_v1.0.pl [-u <mysql user>] [-p <mysql password>] [-r RPD|RPV|RPP] <InputDBList> <PollutantMappingFile> <PollutantFormulasFile> [<OutputPath>]
# where
...
#   RPD|RPP|RPV - optional type of emission factors to process (rate-per-distance, rate-per-vehicle, or rate-per-profile); if not specified, script will process all three types
...
if ($runType && $runType ne 'RPD' && $runType ne 'RPV' && $runType ne 'RPP')
{
  die "Please specify a valid type after '-r': RPD, RPV, or RPP. To run all types do not use '-r' argument.\n";
}

(scalar(@ARGV) >= 3) or die "Usage: $0 [-u <mysql user>] [-p <mysql password>] [-r RPD|RPV|RPP] <InputDBList> <PollutantMappingFile> <PollutantFormulasFile> [<OutputPath>]\n";
...
for my $typeInfo (['RPD', 'distance'], ['RPV', 'vehicle'], ['RPP', 'profile'])
{
